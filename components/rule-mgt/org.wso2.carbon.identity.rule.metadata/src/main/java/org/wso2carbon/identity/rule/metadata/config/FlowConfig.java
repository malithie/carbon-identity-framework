...
public class FlowConfig {
    ...
    public List<FieldDefinition> getFieldDefinitionsForFlow(FlowType flowType) {
        ...
        return fieldDefinitionsMap.get(flowType.getConfigName());
    }

    public static FlowConfig load(String filePath, FieldDefinitionConfig fieldDefinitionConfig)
            throws RuleMetadataConfigException {

        try {
            ObjectMapper mapper = new ObjectMapper();
            Map<String, List<String>> fieldsDefinitionMapPerFlow = mapper.readValue(new File(filePath),
            ...
            throw new RuleMetadataConfigException("Error while loading flows from file: " + filePath, e);
        }
    }
    ...
    private static void validateFlow(String flowName) throws RuleMetadataConfigException {
        ...
        if (!isValidFlow(flowName)) {
            throw new RuleMetadataConfigException("Invalid flow: " + flowName);
        }
    }

    private static boolean isValidFlow(String flowName) {

        for (FlowType flowType : FlowType.values()) {
            if (flowType.getConfigName().equals(flowName)) {
                return true;
            }
        }
        return false;
