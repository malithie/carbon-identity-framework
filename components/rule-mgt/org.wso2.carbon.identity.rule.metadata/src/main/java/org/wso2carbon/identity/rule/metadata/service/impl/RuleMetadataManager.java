...
package org.wso2carbon.identity.rule.metadata.service.impl;
    ...
    private void addFieldDefinitions(RuleMetadataProvider metadataProvider,
        ...
        for (FieldDefinition fieldDefinition : fieldDefinitionsFromProvider) {
            if (addedFieldNames.contains(fieldDefinition.getField().getName())) {
                throw new RuleMetadataServerException(
                        "Duplicate field name: " + fieldDefinition.getField().getName() +
                                " found in the metadata provider: " + metadataProvider.getClass().getName());
            }
